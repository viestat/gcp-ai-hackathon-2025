                ...
                if "text" in message:
                    ...
                    if isinstance(data, dict):
                        ...
                        if "setup" in data:
                            ...
                            logging.debug("Setup message received; not forwarding")
                            continue
...
def get_connect_and_run_callable(
    ...
    async def connect_and_run() -> None:
        if config["use_remote_agent"]:
            # Remote agent engine mode
            logging.info("Connecting to remote agent engine")
            remote_config = {
            ...
            logging.info("Starting local agent engine")
        ...
        logging.info("Starting agent communication")
        await asyncio.gather(
    ...
    if args.mode == "remote":
        ...
        if not args.remote_id:
            ...
            if deployment_metadata_path.exists():
                with open(deployment_metadata_path) as f:
                    ...
                    logging.info("Loaded remote agent engine ID from metadata")
            else:
                parser.error(
        ...
        if not args.project_id:
            ...
            if match:
                ...
                logging.info("Extracted project and location from engine ID")
            else:
                # Fall back to google.auth.default()
                try:
                    _, config["project_id"] = google.auth.default()
                    config["location"] = args.location
                    logging.info("Using default project ID from google.auth")
                except Exception as e:
        ...
        logging.debug(f"Remote ID: {config['remote_agent_engine_id']}")
        logging.debug(f"Project: {config['project_id']}")
        logging.debug(f"Location: {config['location']}")
    else:
        logging.info("Mode: LOCAL")
        logging.debug(f"Agent: {config['local_agent_path']}")
        logging.debug(f"Engine class: {config['agent_engine_class_path']}")
